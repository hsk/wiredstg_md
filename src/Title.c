...
void TitleUpdate(void) {
    if (titleState==0) {// 初期化の開始
        ...
        titleState++;       // 初期化の完了
        VDP_setTileMapDataRectEx(BG_A, title_map,TILE_USER_INDEX, 8, 7, 24, 3, 24, CPU);

//        VDP_setTileMapDataRectEx(BG_A,TILE_USER_INDEX,4,7);
        VDP_drawText("PRESS START BUTTON",7+4,18);
    }
    random();// 乱数の更新
